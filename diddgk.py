...
class diddgk:
    ...
    def AdjMatrixLoss(self, logits, labels):
        #### ここを普通のddgkは交差エントロピーとるところを二乗誤差損失にする．(重みを当てに行く)
    ...
    def MakeEncoder(self, source, ckpt_prefix, hparams):
        ...
        with g.as_default(), session.as_default():
            ##################### 定義
            ######### データを整形
            A = nx.adjacency_matrix(source, weight='weight') # 重み込みの隣接行列 i, j要素がiからjへのエッジ
    ...
    def Measure(self, source, target, ckpt_prefix, hparams):
        ...
        with g.as_default(), session.as_default():
            ########################## モデル定義
            A = nx.adjacency_matrix(target, weight='weight') # 重み込みの隣接行列 i, j要素がiからjへのエッジ
